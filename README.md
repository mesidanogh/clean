...
以下の2社は、店舗ごとにどのプライズを取り扱っているかを公式サイトで公開しており、このアプリはGitHub Actionsで毎日自動巡回してデータを取得し、`data/prizes.json` として保存・検索できるようにしています。ここは完全に自動で、登録作業は不要です。

- **namco**（バンダイナムコアミューズメント直営店、全国220店舗）: 店舗ページに入荷プライズ一覧が掲載
- **タイトー**（タイトーステーションなど、全国250店舗以上）: 商品ごとの取扱店舗を返す公開APIから取得

全国全店舗のデータを持つとデータ量が非常に重くなるため、**東海地方(愛知・岐阜・三重・静岡)の店舗だけ**に絞って保存しています。対象地域を変えたい場合は `scripts/scrape_prizes.py` の `TOKAI_KEYWORDS` を書き換えてください。

一方、GiGO（旧セガ系）・アドアーズなど他チェーンや独立店舗は、店舗ごとの入荷情報を公開していないため自動取得ができません。これらの店舗で見かけた景品は、アプリ内の「＋」ボタンから自分や友達が手動で記録できます（自動データへの補足という位置づけです）。
...
`.github/workflows/scrape.yml` により、毎日自動でnamco・タイトー店舗の最新データを取得して `data/prizes.json` を更新・pushする仕組みがpush時点で既に有効になっています。すぐに最新化したい場合は、リポジトリの「Actions」タブ →「景品データを自動更新」→「Run workflow」で手動実行もできます。
